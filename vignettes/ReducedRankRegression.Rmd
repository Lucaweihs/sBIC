...
In a reduced rank regression model, a vector $Y$ of length $N$ depends on a vector $X$ of length $M$ via the conditional expectation $\mbox{E}(Y \mid X) = B X$. It is assumed that $\text{E}(X) = 0$, $\text{E}(Y) = 0$. The regression model is ``reduced rank'' because the matrix $B$ of regression coefficients may not be of full rank. Possible values of the rank of $B$ range from $0$ ($X$ and $Y$ are independent) to $\min(M,N)$ (the full rank model). If the rank is assumed to be $i$ then model singularities occur when the true data-generating model has rank $j < i$.

## Simulating reduced rank regression data

The `simRR()` function simulates `n` observations from a reduced rank regression model of rank `r`. The parameters `N` and `M` determine the length of $Y$ and $X$, respectively; `n` is the number of observations; and `r` is the rank of the coefficient matrix $B$. The output is a list of matrices `X` and `Y` such that each *column* represents an individual observation (i.e. `X` is transposed with respect to the design matrix used in `lm()`)
...
The output from the `ReducedRankRegressions()` function is combined with the data in the `sBIC()` function to produce the singular BIC as well as the Shwarz BIC. Here we use a simulated data set with 100 observations.
...
Figure 1 shows the frequency of the rank selections by BIC and sBIC as the sample size increases. BIC clearly under-fits for small sample sizes, although it does converge to the true rank of 5 for large samples (n=1000). Singular BIC converges more quickly (n=300). Figure 1 partly reproduces Figure 2 of Drton and Plummer (2017). The latter also includes the Widely Applicable Bayesian Information Criterion (WBIC) of Watanabe (2013).
